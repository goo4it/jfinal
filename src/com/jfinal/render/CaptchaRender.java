...
public class CaptchaRender extends Render {
    ...
    public void render() {
        ...
        captchaName = getProperty("app.captcha", captchaName);
        Cookie cookie = new Cookie(captchaName, vCode);
        cookie.setMaxAge(-1);
        cookie.setPath("/");
        domain = getProperty("app.domain", null);
        if (StrKit.notBlank(domain)) cookie.setDomain(domain);
        try {
    ...
    public static boolean validate(Controller controller, String userInputCaptcha) {
        ...
        boolean result = userInputCaptcha.equalsIgnoreCase(controller.getCookie(captchaName));
        if (result == true) {
            controller.removeCookie(captchaName, "/", domain);
        }
        return result;
    }
}
