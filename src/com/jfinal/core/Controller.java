...
public abstract class Controller {
    ...
    private Map<String, Object>   flashAttribute;
    ...
    private static final String   FLASH_ATTRIBUTE_PREV = "_FLASH_ATTRIBUTE_";
    ...
    void loadFlashAttr() {
        flashAttribute = new HashMap<String, Object>();
        Enumeration<String> names = request.getSession().getAttributeNames();
        while (names.hasMoreElements()) {
            String name = names.nextElement();
            if (name.startsWith(FLASH_ATTRIBUTE_PREV)) {
                flashAttribute.put(name.substring(FLASH_ATTRIBUTE_PREV.length()),
    ...
    public Controller setFlashAttr(String name, Object value) {
        setSessionAttr(FLASH_ATTRIBUTE_PREV + name, value);
        return this;
    }

    public Enumeration<String> getFlashNames() {
        return new Enumeration<String>() {
            private Iterator<String> it = flashAttribute.keySet().iterator();
     ...
     *         of
     *         a request parameter; or an empty Enumeration if the request has
     *         no parameters
     */
     ...
     * request
     * parameter has, or null if the parameter does not exist. If the parameter
     * has a
    ...
    public <T> T getFlashAttr(String name) {
        return (T) flashAttribute.get(name);
    }

    /**
     * Returns an Enumeration containing the names of the attributes available
     * to this request.
     * This method returns an empty Enumeration if the request has no attributes
     * available to it.
     * 
    ...
    private Controller doSetCookie(String name, String value, int maxAgeInSeconds, String path, String domain,
            Boolean isHttpOnly) {
    ...
    public List<UploadFile> getFiles(String uploadPath) {
        if (request instanceof MultipartRequest == false) request = new MultipartRequest(request, uploadPath);
        return ((MultipartRequest) request).getFiles();
    }
    ...
    public List<UploadFile> getFiles() {
        if (request instanceof MultipartRequest == false) request = new MultipartRequest(request);
        return ((MultipartRequest) request).getFiles();
    }
    ...
    public Controller keepPara(Class type, String... names) {
        if (type == String.class) return keepPara(names);
    ...
    public void checkUrlPara(int minLength, int maxLength) {
        getPara(0);
        if (urlParaArray.length < minLength || urlParaArray.length > maxLength) renderError(404);
    }
    ...
    public <T> T enhance(Class<T> targetClass, Class<? extends Interceptor> injectIntersClass1,
            Class<? extends Interceptor> injectIntersClass2) {
        return (T) Enhancer.enhance(targetClass, injectIntersClass1, injectIntersClass2);
    }

    public <T> T enhance(Class<T> targetClass, Class<? extends Interceptor> injectIntersClass1,
            Class<? extends Interceptor> injectIntersClass2, Class<? extends Interceptor> injectIntersClass3) {
    ...
    public <T> T enhance(Object target, Class<? extends Interceptor> injectIntersClass1,
            Class<? extends Interceptor> injectIntersClass2) {
